...
fn main() {
    ...
    loop {
        ...
        let bit_set: u16 = cpu.mem_read(cpu.registers[LC3CPURegister::PC]);
        ...
        match instruction {
            ...
            LC3Instruction::JMP => {
                ...
                cpu.registers[LC3CPURegister::PC] = cpu.registers[base_register];
            } /* jump */
            LC3Instruction::LD => {
                let dr = (bit_set >> 9) & 0x7;
                let pc_offset_9 = sign_extend(bit_set & 0x1FF, 9);
                cpu.registers[dr] = cpu.mem_read(cpu.registers[LC3CPURegister::PC] + pc_offset_9);
                cpu.update_flags(dr);
            } /* load */
            LC3Instruction::ST => {
            ...
            LC3Instruction::JSR => {
                ...
                match mode {
                    ...
                    REGISTER_MODE => {
                        ...
                        cpu.registers[LC3CPURegister::PC] += cpu.registers[base_r];
                    }
                    _ => panic!("Invalid mode"),
                }
            } /* jump register */
            LC3Instruction::LDR => {
                let dr = (bit_set >> 9) & 0x7;
                let base_r = (bit_set >> 6) & 0x7;
                let offset_6 = sign_extend(bit_set >> 0x3F, 6);
                cpu.registers[dr] = cpu.mem_read(cpu.registers[base_r] + offset_6);
                cpu.update_flags(dr);
            } /* load register */
            LC3Instruction::STR => {
                let sr = (bit_set >> 9) & 0x7;
                let base_r = (bit_set >> 6) & 0x7;
                let offset_6 = sign_extend(bit_set >> 0x3F, 6);
                cpu.mem_write(cpu.registers[base_r] + offset_6, cpu.registers[sr]);
            } /* store register */
            LC3Instruction::NOT => {
                let dr = (bit_set >> 9) & 0x7;
                let sr = (bit_set >> 6) & 0x7;
            ...
            LC3Instruction::LDI => {
                ...
                let pc_offset_9 = sign_extend(bit_set & 0x1FF, 9);
                cpu.registers[dr] =
                    cpu.mem_read(cpu.mem_read(cpu.registers[LC3CPURegister::PC] + pc_offset_9));
                cpu.update_flags(dr);
            } /* load indirect */
            LC3Instruction::STI => {
                let sr = (bit_set >> 9) & 0x7;
                let pc_offset_9 = sign_extend(bit_set & 0x1FF, 9);
                cpu.mem_write(
                    cpu.mem_read(cpu.registers[LC3CPURegister::PC] + pc_offset_9),
                    cpu.registers[sr],
                );
            } /* store indirect */
            LC3Instruction::LEA => {
                let dr = (bit_set >> 9) & 0x7;
                let pc_offset_9 = sign_extend(bit_set & 0x1FF, 9);
                cpu.registers[dr] = cpu.registers[LC3CPURegister::PC] + pc_offset_9;
                cpu.update_flags(dr);
            ...
            LC3Instruction::TRAP => {
                let trapvect8 = bit_set & 0xFF;
                TrapRoutine::execute(&cpu, trapvect8);
            } /* execute trap */
        }
    }
}
